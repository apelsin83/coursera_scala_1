...
object Anagrams {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y foldLeft SortedMap[Char,Int]() ++ x){ case (map, (ch, cnt)) => {
      val newCnt = map(ch) - cnt
      if (newCnt != 0) map updated (ch, newCnt)
      else map - ch
    } }.toList
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def traverse(occur: Occurrences): List[Sentence] = {
    if (occur.isEmpty) List(Nil)
    else for {
      comb <- combinations(occur)
      word <- dictionaryByOccurrences getOrElse (comb, Nil)
      sentence <- traverse(subtract(occur, wordOccurrences(word)))
      if comb.nonEmpty
    } yield word::sentence
